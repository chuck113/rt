...
object ScalaConversions {
  ...
  def addKeys[K,V](map: Map[K, V], keys:List[K], newValue:V): Map[K, V]={
    val res = collection.mutable.Map[K, V]() ++ map;
    for(key <- keys){
      res(key) = newValue
    }
    Map.empty ++ res
  ...
  def toMap[K,V](list:List[K], mapFunction:(K => V)):Map[K,V]={
  ...
  def mergeListMaps[K,V](target: Map[K, List[V]], mapTwo: Map[K, List[V]]): Map[K, List[V]] = {
    val res = collection.mutable.Map[K, List[V]]() ++ target;
